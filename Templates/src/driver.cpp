...
void mapOfObjects (int startKey, int numberItems)
{
  ...
  std::map<unsigned int, ExampleClass> cTheMap;
  srand (2);
  ...
  for (int i = startKey; i < startKey + numberItems; i++)
  {
    ...
    cTheMap.insert (std::make_pair (value, ExampleClass (value, std::to_string (value))));
    std::cout << "INSERT: " << value << std::endl;
  }


  for (std::map<unsigned, ExampleClass>::iterator it = cTheMap.begin ();
    it != cTheMap.end (); ++it)
  {
